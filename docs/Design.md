...
Datask's marketing canvas is a near-pure black artboard. The dominant surface is `{colors.canvas}` — almost pure black with a faint warmth — and on top of it sits oversized white display type set in **GT Walsheim Medium** with letter-spacing pulled to extreme negative values (-5.5px on the 110px display, -4.25px on the 85px hero). The page reads like a poster: one assertive statement per band, generous breathing room above and below.

The single accent is `{colors.accent-blue}` — used scarcely, mostly for hyperlinks, selection halos, and a subtle blue-tinted shadow ring on focused inputs. The brand chrome itself is monochrome: white pill buttons, charcoal cards, gray secondary text. What makes Datask distinctive is the rhythm break — every few sections the page drops in a **vibrant gradient atmosphere card**: a magenta-violet spotlight, a sunset-orange wash, a coral-pink panel. These aren't section backgrounds; they're individual cards arranged in a card grid, each one a small living poster that shows what Datask can produce.

Body type is **Inter Variable**, with Datask leaning hard into Inter's character variants (`cv01`, `cv05`, `cv09`, `cv11`, `ss03`, `ss07`, `dlig`) — the result is a body voice that feels custom-tuned, with single-storey "a", straight-leg "l", and tabular figures. There's no light mode on the marketing site; the brand IS dark.
...
> Source pages: Datask.com (home), /ai/, /startups/, /marketplace/templates/nudge/, /gallery/a16z-speedrun-×-tonik, /pricing.
...
- **GT Walsheim Datask Medium** / **GT Walsheim Medium** — Datask's display typeface. Geometric, slightly humanist, very confident at large sizes with extreme negative tracking. Fallbacks: `GT Walsheim Medium Placeholder` system font.
...
- **Tight line-heights everywhere.** Even body runs at 1.30 — Datask's editorial tone is denser than typical SaaS marketing.

### Note on Font Substitutes

If implementing without GT Walsheim Medium, suitable open-source substitutes include **Mona Sans**, **Geist**, or **Inter** at weight 600–700 with manually tightened tracking. Mona Sans's hairline weights at 100–300 are particularly close to Datask's cleaner section openers. Inter Variable is open-source — keep it as-is and preserve the documented OpenType variants.

## Layout

### Spacing System

- **Base unit**: 5px (Datask uses non-standard 5/10/15/20/30 increments rather than the more common 4/8/16/24).
...
The dark canvas IS the whitespace. Where lighter brands lean on white air to separate sections, Datask leans on long stretches of black with a single oversized statement floating in the middle. Sections separate by mode change: a band of charcoal cards, then a band of black with a gradient spotlight, then back to charcoal — like cuts in a dark film.
...
- **Layered product mockups** (browser frames containing live Datask-built sites) sit inside `{colors.surface-1}` cards with the level-2 light-edge treatment.
...
Datask's extracted radius set is unusually granular (1px, 4px, 5px, 6px, 8px, 10px, 12px, 15px, 20px, 30px, 40px, 100px). The named scale below picks the levels the marketing surface actually consumes.
...
- Embedded site mockups (browser-chromed previews of Datask-built sites) sit in `{rounded.xl}` 20px tiles with `{spacing.md}` 15px interior padding.
...
**`template-card`** — Thumbnail tile in the home "Built with Datask" gallery and `/marketplace`.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body-sm}`, rounded `{rounded.lg}`, padding 12px.

**`product-mockup-tile`** — Larger tile that frames a live product UI mock (Datask canvas, Workshop video, AI translate panel).
...
The defining decorative surface of Datask's marketing — oversized atmospheric tiles dropped into otherwise monochrome card grids. Variants:
...
**`top-nav`** — Sticky bar on `{colors.canvas}` with the Datask wordmark left, primary nav links centered, and a `button-secondary` ("Sign in") + `button-primary` ("Get started for free") pair right.
...
**`footer`** — Dense link grid on `{colors.canvas}` with the Datask wordmark left and 5–6 columns of caption-sized links.
...
- Don't ship a light-mode marketing page. Datask's identity is dark.
...
- Embedded product mockups (browser frames containing live Datask-built sites) maintain their aspect ratio and never crop.
