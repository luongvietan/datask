# -*- coding: utf-8 -*-
"""
Datask benchmark runner — fetches all sites from sites.csv via Datask API.

Usage:
    DATASK_API_KEY=dtsk_live_... uv run python benchmarks/turnstile-2026/run_datask.py

Env vars:
    DATASK_API_KEY   - required, Datask API key
    DATASK_BASE_URL  - optional, defaults to https://api.datask.run
    DATASK_CONCURRENCY - optional, max parallel requests (default: 3)
    DATASK_RETRIES   - optional, retries per site on failure (default: 2)
    DATASK_TIMEOUT   - optional, request timeout in seconds (default: 60)
"""
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                result["success"] = len(content) >= MIN_CONTENT_LENGTH

                if result["success"]:
                    result["error"] = None
                    break
                else:
                    result["error"] = f"Content too short ({len(content)} chars < {MIN_CONTENT_LENGTH})"
...
def run_benchmark(
    ...
    with (
        ...
        futures = {
            pool.submit(fetch_single, client, site["url"], retries, timeout): site
            for site in sites
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Datask benchmark on Turnstile sites")
    parser.add_argument("--base-url", default=os.environ.get("DATASK_BASE_URL", "https://api.datask.run"))
    parser.add_argument("--concurrency", type=int, default=int(os.environ.get("DATASK_CONCURRENCY", "3")))
    parser.add_argument("--retries", type=int, default=int(os.environ.get("DATASK_RETRIES", "2")))
    parser.add_argument("--timeout", type=float, default=float(os.environ.get("DATASK_TIMEOUT", "60")))
    parser.add_argument("--output", default=None, help="Output JSON file path")
    args = parser.parse_args()

    api_key = os.environ.get("DATASK_API_KEY", "")
    if not api_key:
        print("Error: DATASK_API_KEY env var required", file=sys.stderr)
        sys.exit(1)

    sites = load_sites()
    print(f"Datask Benchmark — {len(sites)} sites")
    ...
    output_path = args.output or str(RESULTS_DIR / "datask_results.json")
    output = {
        "provider": "datask",
